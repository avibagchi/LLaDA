#!/usr/bin/env python3
"""
dgMARK ablation sweep — loads LLaDA and GPT-2 ONCE, then iterates over all
(gamma, delta, tend) combinations without reloading.

dgMARK steers the UNMASKING ORDER (not logits) using a parity constraint.
gamma is not used in dgMARK generation (no green list), but the grid is kept
consistent.  delta is passed through for config logging only — dgMARK has no
amplification parameter.  The key hyperparameter is t_end.

Usage:
    python run_dgmark_ablation_efficient.py --device cuda:3
    python run_dgmark_ablation_efficient.py --device cuda:3 --resume
"""
import os
...
from generate import generate, calculate_dgmark_score, get_special_token_ids

# dgMARK has no gamma/delta hyperparameters in generation, but we keep the
# grid consistent.  We ablate only over t_end; gamma/delta are logged as 0.5/0.
GAMMA_VALUES = [0.5]          # placeholder (not used by dgMARK generation)
DELTA_VALUES = [0.0]          # placeholder (not used by dgMARK generation)
    ...
    for idx, (entry, prompt_tokens) in enumerate(zip(entries, prompt_tokens_list)):
        ...
        with torch.no_grad():
            generated = generate(
                ...
                mask_id=mask_id,
                watermark_type='dgmark',
                vocab_size=vocab_size,
                special_token_ids=special_token_ids,
                dgmark_seed=dgmark_seed,
...
def main():
    ...
    combos = list(TEND_VALUES)
    if args.resume:
        combos = [t for t in combos
                  if not (output_dir / f"dgmark_tend={t}_sampled_100.json").exists()]
        print(f"Resume mode: {len(combos)} combinations remaining")
    else:
        print(f"Running {len(combos)} t_end values")

    total = len(combos)
    for combo_idx, tend in enumerate(combos):
        out_name = f"dgmark_tend={tend}_sampled_100.json"
        out_path = output_dir / out_name
        print(f"\n[{combo_idx+1}/{total}] t_end={tend} → {out_name}")

        results = run_combination(
            model, tokenizer, entries, prompt_tokens_list,
            eval_model, eval_tokenizer, special_token_ids,
            tend=tend, dgmark_seed=args.dgmark_seed,
