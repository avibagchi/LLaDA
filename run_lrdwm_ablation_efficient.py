#!/usr/bin/env python3
"""
LR-DWM ablation sweep — loads LLaDA and GPT-2 ONCE, then iterates over all
(gamma, delta, tend) combinations without reloading.

LR-DWM uses TWO independent hash keys (k_L = seed, k_R = seed XOR constant)
for left and right neighbor green lists.  Only actually-revealed neighbors are
used (no predictions).  Detection uses s_i = m_L + m_R - 1 for interior
positions with both neighbors.

Usage:
    python run_lrdwm_ablation_efficient.py --device cuda:3
    python run_lrdwm_ablation_efficient.py --device cuda:3 --resume
"""
import os
...
from generate import generate, calculate_lrdwm_score, get_special_token_ids
    ...
    for idx, (entry, prompt_tokens) in enumerate(zip(entries, prompt_tokens_list)):
        ...
        with torch.no_grad():
            generated = generate(
                ...
                mask_id=mask_id,
                watermark_type='lrdwm',
                gamma=gamma,
                amplification=delta,
                vocab_size=vocab_size,
                special_token_ids=special_token_ids,
                lrdwm_seed=lrdwm_seed,
...
def main():
    ...
    if args.resume:
        combos = [
            ...
            if not (output_dir / f"lrdwm_gamma={g}_delta={d}_tend={t}_sampled_100.json").exists()
        ]
    ...
    for combo_idx, (gamma, delta, tend) in enumerate(combos):
        out_name = f"lrdwm_gamma={gamma}_delta={delta}_tend={tend}_sampled_100.json"
