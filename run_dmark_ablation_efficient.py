#!/usr/bin/env python3
"""
DMark ablation sweep — loads LLaDA and GPT-2 ONCE, then iterates over all
(gamma, delta, tend) combinations without reloading.

Equivalent to running eval_waterbench.py 175 times but ~10x faster because
the ~60s model-load overhead only happens once.

Usage:
    python run_dmark_ablation_efficient.py                        # default settings
    python run_dmark_ablation_efficient.py --variant predictive
    python run_dmark_ablation_efficient.py --device cuda:3
    python run_dmark_ablation_efficient.py --resume              # skip existing output files
"""
import torch
import json
import math
...
from generate import generate, calculate_dmark_score, get_special_token_ids

# ---------------------------------------------------------------------------
# Ablation grid (matches Kirchenbauer / Gloaguen ablation structure)
# ---------------------------------------------------------------------------
...
def run_combination(model, tokenizer, entries, prompt_tokens_list,
                    ...
                    gamma, delta, tend, variant, dmark_seed,
...
def main():
    ...
    parser.add_argument('--jsonl_file', type=str,
                        default='water-bench-sampled_100_seed43.jsonl')
    parser.add_argument('--output_dir', type=str,
                        default='water-bench-results/json-outputs')
    parser.add_argument('--model_path', type=str,
                        default='GSAI-ML/LLaDA-8B-Instruct')
    parser.add_argument('--device', type=str, default='cuda:3')
    parser.add_argument('--variant', type=str,
                        default='predictive_bidirectional',
    ...
    # ---- Load and preprocess prompts ONCE ----
    ...
    if args.resume:
        combos = [
            ...
            if not (output_dir / f"dmark_{args.variant}_gamma={g}_delta={d}_tend={t}_sampled_100.json").exists()
        ]
        print(f"Resume mode: {len(combos)} combinations remaining")
    else:
        print(f"Running {len(combos)} combinations (variant={args.variant})")

    total = len(combos)
    for combo_idx, (gamma, delta, tend) in enumerate(combos):
        out_name = f"dmark_{args.variant}_gamma={gamma}_delta={delta}_tend={tend}_sampled_100.json"
        ...
        results = run_combination(
            ...
            variant=args.variant, dmark_seed=args.dmark_seed,
    ...
    print("Find optimal hyperparams:")
